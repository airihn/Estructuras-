# Nodo para lista simple de trabajos disponibles
class NodoTrabajo:
...
class ListaTrabajos:
    ...
    def eliminar(self, puesto):
        ...
        while actual:
            if actual.puesto == puesto:
                if previo is None: # Es el primer nodo
    ...
    def _contar_recursivo(self, nodo):
        ...
        if nodo is None:  # llegamos al final
            return 0
        return 1 + self._contar_recursivo(nodo.siguiente)  # Suma 1 y sigue

# Nodo y lista doble para el historial de un estudiante

class NodoHistorial:
    """Nodo doblemente enlazado para el historial de solicitudes."""
    def __init__(self, evento):
        self.evento = evento  # descripción del evento
        self.siguiente = None
        self.anterior = None
